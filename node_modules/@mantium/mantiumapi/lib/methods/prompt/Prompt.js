...
module.exports = function (headers, opt) {
  ...
  if (['POST', 'PATCH'].includes(opt.method) && !utils.isNotNilOrEmptyString(opt.ai_provider)) throw new Error(msg.errorMessages().Provider_missing);
  ...
  var url = config.promptURL(opt.isIdURL).concat('/', id, utils.objToQueryStr(opt.queryParam));
  ...
  if (utils.isNotNil(opt.action) && opt.action === 'try') {
    ...
    url = config.promptTryURL(opt.id).concat(utils.objToQueryStr({
      input: opt.input
    }));
  }
